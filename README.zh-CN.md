...
- **会话管理**：分组树（嵌套分组、拖拽移动）、标签、收藏、模糊搜索、命令面板（Ctrl+Shift+P）；密码 / 公钥（OpenSSH + .ppk）/ keyboard-interactive（2FA）/ agent 认证；多级跳板机；known_hosts 首次连接与密钥变更弹窗确认；可选 su 二级登录——普通用户登入后自动切换到目标账号（如 root），密码存保险库并在提示出现时自动应答一次
