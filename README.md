...
- **Sessions**: nested group tree (drag & drop), tags, favorites, fuzzy search, command palette (Ctrl+Shift+P); password / public-key (OpenSSH & .ppk) / keyboard-interactive (2FA) / agent auth; multi-hop ProxyJump; known_hosts confirmation on first connect and key change; optional su second-login — sign in as a regular user and auto-switch to another account (e.g. root) with a stored password answered once at the prompt
