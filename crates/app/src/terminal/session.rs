//! 终端会话生命周期 module（卡 2）：I/O 半抽象、su 二级登录、登录宏、
//! supervise/reconnect 监督循环。热循环在 stream.rs；命令与管理器在 terminal.rs。
...
use core_ssh::{ConnectOptions, PtyReader, PtyWriter, SshConnection};

use super::stream::{read_loop, RingBuf};
use super::TerminalManager;
use crate::stream_protocol::CreditState;

/// 读半抽象：SSH 通道 或 本地 PTY（批次十四 本地会话）
pub(super) enum AnyReader {
    Ssh(PtyReader),
    Local(crate::local_pty::LocalReader),
}

impl super::stream::ByteSource for AnyReader {
...
pub(super) enum AnyWriter {
    Ssh(PtyWriter),
    Local(crate::local_pty::LocalWriter),
}

impl AnyWriter {
    pub(super) async fn write(&self, data: &[u8]) -> Result<(), String> {
        match self {
    ...
    pub(super) async fn resize(&self, cols: u32, rows: u32) -> Result<(), String> {
        match self {
...
pub(super) struct SshReconnect {
    pub(super) opts: ConnectOptions,
...
impl SuWatch {
    ...
    pub(super) fn is_password_prompt(chunk: &[u8]) -> bool {
        const MARKERS: [&[u8]; 4] = [
            b"assword",
            "密码".as_bytes(),
...
pub(super) struct MacroPending {
    pub(super) lines: Vec<String>,
    pub(super) input_enc: Option<Arc<Mutex<crate::encoding::InputEncoder>>>,
}
...
pub(super) fn macro_lines(raw: &str) -> Vec<String> {
    raw.lines()
        .map(str::trim)
        .filter(|l| !l.is_empty())
        .map(str::to_owned)
...
pub(super) struct TermSession {
    /// 重连时整枚替换（sessions 锁内 swap）
    pub(super) writer: Arc<AnyWriter>,
...
pub(super) struct SuperviseCtx {
    pub(super) tab_id: String,
    pub(super) mgr: Arc<TerminalManager>,
    /// 随会话隧道停止所需的归属与句柄（session_id 为 None 时短路）
    pub(super) session_id: Option<String>,
    pub(super) tunnel_mgr: Arc<core_tunnel::TunnelManager>,
    pub(super) store: Arc<core_store::Store>,
    pub(super) backend: Backend,
    /// su 二级登录（批次二十二）：None = 不启用；重连后自动重放
    pub(super) su: Option<SuConfig>,
    /// 登录宏（已解析行）：每轮 shell（首连/重连）重放；空 = 无
    pub(super) login_macro: Vec<String>,
    /// 输入转码器（宏文本非 utf-8 会话转码用；None = 直通）
    pub(super) input_enc: Option<Arc<Mutex<crate::encoding::InputEncoder>>>,
    /// 当前写半（重连即换）：su 命令与密码应答的写入通道
    pub(super) writer: Arc<AnyWriter>,
    /// 输出转码（非 utf-8 会话）：读循环内建 Decoder，重连即重建
    pub(super) out_encoding: Option<&'static encoding_rs::Encoding>,
    pub(super) data: Channel<Response>,
    pub(super) events: Channel<Value>,
    pub(super) credit: Arc<CreditState>,
    /// 前台标记与 ring（PR-17 二期；与 TermSession 共享 Arc）
    pub(super) focused: Arc<AtomicBool>,
    pub(super) ring: Arc<RingBuf>,
    pub(super) reader: AnyReader,
}

pub(super) async fn supervise(mut ctx: SuperviseCtx) {
    loop {
